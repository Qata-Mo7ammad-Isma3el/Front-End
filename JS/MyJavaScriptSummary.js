...
? Scopes In JS : 
  ...
  * These Variables can be accessed from anywhere within the same Javascript code. 
  * var, let and const all provide this Scope.
  ~ 2- Function Scope: 
  * Variables declared within a function can only be used within that same function. 
...
% This method compares two values for strict equality, similar to ===, but with some key differences.
! Key Differences from ===:
~ 1) Object.is(+0, -0) → false
// Unlike ===, it treats +0 and -0 as Not equal.
~ 2) Object.is(NaN, NaN) → true
// Unlike ===, it treats NaN and NaN as equal.
// also you can use (< , <= , > , >=).
...
// do{...}while (condition);
~ 4) for ... in  loop
// it iterates over the enumerable property keys (names) of an object (including inherited enumerable properties).
// for (let key in object) { access key and value }
// Notes : 
! Mostly used with objects, not arrays.
! Order of keys is not guaranteed.
! Will loop over inherited properties unless you check with hasOwnProperty(): if (user.hasOwnProperty(key)) { ... }
~ 5) for ... of loop
// it iterates over iterable values (arrays, strings, maps, sets, etc.) not objects directly.
// for (let value of iterable) { use value directly }
// Notes:
! Not for plain objects, unless the object has a [Symbol.iterator] defined.
! Doesn’t return key/index by default — just the value.
~ 6) .forEach() loop
// it is an array method that executes a provided function once for each element in the array.
// array.forEach(function(element, index, array) { logic here });
// Notes:
! Cannot be stopped with break or continue (unlike for/for...of).
! Does not return a value (always returns undefined).
! More functional/clean, but less flexible for control flow.
...
~ 1) Global Context:
% When used outside any function (in global scope), this refers to the global object:
% In browsers: window
% In Node.js: global
~ 2) Function Context:
...
~ 3) Constructor Context (with new)
% When a function is called with new, this refers to the newly created instance.
~ 4) Explicit Binding You can explicitly set this using (Function Borrowing):
* Function borrowing allows us to use the methods of one object on a different object.
* we don't have to make a copy of that method and maintain it in two separate places.
% a) call()
* Immediately invokes the function
* Sets this to the first argument
* Accepts additional arguments individually (comma-separated)
! func.call(thisArg, arg1, arg2, ...)
% b) apply()
* Immediately invokes the function
* Sets this to the first argument
* Accepts additional arguments as an array
! func.apply(thisArg, [arg1, arg2, ...])
% c) bind()
* Creates a new function with permanently bound this
* Does not immediately invoke the function
* Accepts arguments individually (comma-separated)
* Useful for event handlers or callbacks where you want fixed context
...
~ 7) Class Context : 
% In class constructors and methods, this refers to the instance.
...
// The main Syntax of Exception handling with try/catch/finally : 
% try {
...
~ TypeError: Using a value in an unexpected type way.
~ SyntaxError: Incorrect code syntax.
~ RangeError: A value not in the allowed range.
~ EvalError: Issues with the eval() function (rare).
~ URIError: Errors in encodeURI() / decodeURI().
...
! Note :
* Optional catch Binding (ES10+) From ES10 (ECMAScript 2019) onward, you can omit the error variable:
% try {
%   some risky code
% } catch {
